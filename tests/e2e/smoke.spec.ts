...
test("connect -> login -> home -> detail -> player -> search with propagation", async ({ page }) => {
  await installTauriMock(page);
  await page.goto("/", { waitUntil: "domcontentloaded" });

  await expect(page.getByRole("heading", { name: "jfgoat" })).toBeVisible();

  await page.getByPlaceholder("http://your-server:8096").fill("http://demo.local");
  ...
  await expect(page.getByRole("button", { name: "Open details for Pilot" })).toBeVisible();

  await page.getByRole("button", { name: "Open details for Pilot" }).click();
  await expect(page.getByRole("heading", { name: /S1 - E1 - Pilot/ })).toBeVisible();
  ...
  await expect(page.getByRole("button", { name: "Open details for Pilot" })).toHaveCount(0);
