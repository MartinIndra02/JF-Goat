...
<main class="min-h-screen bg-gray-900 text-white">
  ...
  <div class="relative w-full overflow-hidden" style="height: clamp(340px, 55vh, 560px);">
    ...
    <div class="absolute top-4 right-4 z-10 flex items-center gap-1.5">
      <button class="p-2 bg-black/40 hover:bg-black/60 rounded-full backdrop-blur-sm transition-colors text-gray-300" aria-label="Download">
      ...
      <button class="p-2 bg-black/40 hover:bg-black/60 rounded-full backdrop-blur-sm transition-colors text-gray-300" aria-label="Layout">
      ...
      <button class="p-2 bg-black/40 hover:bg-black/60 rounded-full backdrop-blur-sm transition-colors text-gray-300" aria-label="Sync">
  ...
  <div class="relative -mt-32 z-10 px-5 pb-16 max-w-3xl mx-auto">
    ...
    <div class="flex items-center justify-center gap-2 mb-5">
      <button aria-label="Toggle favorite" onclick={() => onToggleFavorite(item.id, item.is_favorite)} class="p-2.5 rounded-full hover:bg-white/10 transition-colors {item.is_favorite ? 'text-red-400' : 'text-gray-400'}">
      ...
      <button aria-label="Toggle watched" onclick={() => onTogglePlayed(item.id, item.played)} class="p-2.5 rounded-full hover:bg-white/10 transition-colors {item.played ? 'text-green-400' : 'text-gray-400'}">
      ...
      <!-- Context menu -->
      <div class="relative">
        <button aria-label="More options" onclick={() => contextMenuOpen = !contextMenuOpen} class="p-2.5 rounded-full hover:bg-white/10 transition-colors text-gray-400">
        ...
        <div bind:this={siblingScrollEl} class="flex gap-3 overflow-x-auto pb-4 scrollbar-hide -mx-5 px-5">
          {#each siblingEpisodes as episode (episode.id)}
            <button
              ...
              class="flex-shrink-0 w-48 rounded-xl overflow-hidden bg-white/5 hover:bg-white/10 transition-colors text-left cursor-pointer {episode.id === item?.id ? 'ring-2 ring-blue-500' : ''}"
            >
              <div class="relative">
