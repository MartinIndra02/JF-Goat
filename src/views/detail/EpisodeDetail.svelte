<script lang="ts">
  ...
  async function handleConfirmDownload(quality: "original" | "720" | "480" | "360") {
    ...
    const subtitleIndices = mediaStreams ? mediaStreams.subtitle.map(s => s.index) : [];
    ...
    const streamsJson = mediaStreams ? JSON.stringify(mediaStreams) : null;

    try {
      await startDownload(item.id, audioIndices, subtitleIndices, height, bitrate, streamsJson);
    } catch (e) {
